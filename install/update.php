...
function pluginMonitoringGetCurrentVersion($version) {
    ...
    if ((!TableExists("glpi_plugin_monitoring_configs"))) {
        ...
        if ($DB->numrows($result) > 0) {
            ...
            if (is_null($data['version']) || $data['version'] == '') {
...
function pluginMonitoringUpdate($current_version, $migrationname = 'Migration') {
    ...
    $migration->addField($newTable, 'notification_interval', "int(4) NOT NULL DEFAULT '30'");
    $migration->addField($newTable, 'hostsnotification_id', "int(11) DEFAULT '-1'");
    $migration->addField($newTable, 'servicesnotification_id', "int(11) DEFAULT '-1'");
    $migration->addKey($newTable, "name");
    $migration->migrationOneTable($newTable);
    ...
    $migration->addField($newTable, 'business_priority', "tinyint(1) NOT NULL DEFAULT '3'");
    ...
    $migration->addKey($newTable, array('plugin_monitoring_componentscalalog_id',
    ...
    $migration->addField($newTable, 'acknowledge_comment', "text DEFAULT NULL COLLATE utf8_unicode_ci");
    $migration->addField($newTable, 'acknowledge_users_id', "int(11) NOT NULL DEFAULT '0'");
    $migration->addKey($newTable, array('state',
        'state_type'), 'state');
    ...
    $migration->changeField($newTable, 'host_notification_options_s', 'host_notification_options_s', "tinyint(1) NOT NULL DEFAULT '1'");
    ...
    $migration->addField($newTable, 'host_notification_options_s', "tinyint(1) NOT NULL DEFAULT '1'");
    ...
    $migration->changeField($newTable, 'service_notification_options_s', 'service_notification_options_s', "tinyint(1) NOT NULL DEFAULT '1'");
    ...
    $migration->addField($newTable, 'service_notification_options_s', "tinyint(1) NOT NULL DEFAULT '1'");
    ...
    $migration->changeField($newTable, 'service_notification_options_r', 'service_notification_options_r', "tinyint(1) NOT NULL DEFAULT '1'");
    $migration->changeField($newTable, 'service_notification_options_f', 'service_notification_options_f', "tinyint(1) NOT NULL DEFAULT '0'");
    $migration->changeField($newTable, 'service_notification_options_s', 'service_notification_options_s', "tinyint(1) NOT NULL DEFAULT '0'");
    $migration->changeField($newTable, 'service_notification_options_n', 'service_notification_options_n', "tinyint(1) NOT NULL DEFAULT '0'");
    $migration->changeField($newTable, 'host_notification_options_d', 'host_notification_options_d', "tinyint(1) NOT NULL DEFAULT '1'");
    $migration->changeField($newTable, 'host_notification_options_u', 'host_notification_options_u', "tinyint(1) NOT NULL DEFAULT '1'");
    $migration->changeField($newTable, 'host_notification_options_r', 'host_notification_options_r', "tinyint(1) NOT NULL DEFAULT '1'");
    ...
    $migration->addField($newTable, 'service_notification_options_r', "tinyint(1) NOT NULL DEFAULT '1'");
    $migration->addField($newTable, 'service_notification_options_f', "tinyint(1) NOT NULL DEFAULT '0'");
    $migration->addField($newTable, 'service_notification_options_s', "tinyint(1) NOT NULL DEFAULT '0'");
    $migration->addField($newTable, 'service_notification_options_n', "tinyint(1) NOT NULL DEFAULT '0'");
    $migration->addField($newTable, 'host_notification_options_d', "tinyint(1) NOT NULL DEFAULT '1'");
    $migration->addField($newTable, 'host_notification_options_u', "tinyint(1) NOT NULL DEFAULT '1'");
    $migration->addField($newTable, 'host_notification_options_r', "tinyint(1) NOT NULL DEFAULT '1'");
    ...
    if (count($a_hostsconfig) > 0) {
        ...
        foreach ($a_hostsconfig as $data) {
            $a_components = getAllDatasFromTable(
                'glpi_plugin_monitoring_components', "`plugin_monitoring_commands_id`='" . $data['plugin_monitoring_commands_id'] . "'"
            );
    ...
    if (count($a_hosts) > 0) {
        ...
        foreach ($a_hosts as $data) {
            $a_computers = getAllDatasFromTable(
                'glpi_computers', "`id`='" . $data['items_id'] . "'"
            );
    ...
    $migration->addKey($newTable, array('plugin_monitoring_contacts_id',
        'plugin_monitoring_contactgroups_id'), "unicity", "UNIQUE");
    ...
    $migration->addKey($newTable, array('plugin_monitoring_contactgroups_id_1',
        'plugin_monitoring_contactgroups_id_2'), "unicity", "UNIQUE");
    ...
    if (!TableExists($newTable)) {
        ...
        $migration->addKey($newTable, array('plugin_monitoring_services_id', 'begin_date',
            'end_date'), 'plugin_monitoring_services_id_2');
        $migration->migrationOneTable($newTable);
    }
    /*
      $migration->changeField($newTable,
     ...
     * Table glpi_plugin_monitoring_services
     */
    $newTable = "glpi_plugin_monitoring_dependencies";
    if (!TableExists($newTable)) {
        $query = "CREATE TABLE `" . $newTable . "` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
            ...
            `service_dependency` int(11) DEFAULT NULL,
            PRIMARY KEY (`id`)
        ) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->query($query);
    }
...
function migrateTablesMonitoring($migration, $a_table) {
    ...
    foreach ($a_table['renamefields'] as $old => $new) {
        $migration->changeField($a_table['name'], $old, $new, $a_table['fields'][$new]['type'], array('value' => $a_table['fields'][$new]['value'],
            'update' => TRUE));
    }

    foreach ($a_table['oldfields'] as $field) {
        $migration->dropField($a_table['name'], $field);
    }
    $migration->migrationOneTable($a_table['name']);

    foreach ($a_table['fields'] as $field => $data) {
        $migration->changeField($a_table['name'], $field, $field, $data['type'], array('value' => $data['value']));
    }
    $migration->migrationOneTable($a_table['name']);

    foreach ($a_table['fields'] as $field => $data) {
        $migration->addField($a_table['name'], $field, $data['type'], array('value' => $data['value']));
    }
    $migration->migrationOneTable($a_table['name']);

    foreach ($a_table['oldkeys'] as $field) {
        $migration->dropKey($a_table['name'], $field);
    }
    $migration->migrationOneTable($a_table['name']);
    ...
    foreach ($a_table['keys'] as $data) {
        ...
        $migration->addKey($a_table['name'], $data['field'], $data['name'], $data['type'], $data['len']);
    }
    $migration->migrationOneTable($a_table['name']);
