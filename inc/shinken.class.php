...
class PluginMonitoringShinken extends CommonDBTM {
    ...
    function generateCommandsCfg($file = 0) {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateCommandsCfg ...\n"
        );
        ...
        foreach ($a_list as $data) {
            ...
            $a_commands[$i] = $this->add_value_type(
                PluginMonitoringCommand::$command_prefix . $data['command_name'], 'command_name', $a_commands[$i]);
            ...
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', "- command: " . $a_commands[$i]['command_name'] . " -> " . $a_commands[$i]['name'] . "\n"
            );
        ...
        if (!$restart_shinken_1_4_found) {
            ...
            $a_commands[$i] = $this->add_value_type(
                "nohup sh -c '/usr/local/shinken/bin/stop_arbiter.sh && sleep 3 && /usr/local/shinken/bin/launch_arbiter.sh'  > /dev/null 2>&1 &", 'command_line', $a_commands[$i]);
        }
        if (!$reload_shinken_found) {
            ...
            $a_commands[$i] = $this->add_value_type(
                "nohup sh -c '/etc/init.d/shinken reload' > /dev/null 2>&1 &", 'command_line', $a_commands[$i]);
        }
        if (!$restart_shinken_found) {
            ...
            $a_commands[$i] = $this->add_value_type(
                "nohup sh -c '/etc/init.d/shinken restart' > /dev/null 2>&1 &", 'command_line', $a_commands[$i]);
        }
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateCommandsCfg\n"
        );
    ...
    function generateHostsCfg($file = 0, $tag = '') {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateHostsCfg ($tag) ...\n"
        );
        $pmCommand = new PluginMonitoringCommand();
        ...
        $query = "SELECT
         ...
         CONCAT_WS('', `glpi_computers`.`id`, `glpi_printers`.`id`, `glpi_networkequipments`.`id`, `glpi_peripherals`.`id`) AS id,
         CONCAT_WS('', `glpi_computers`.`entities_id`, `glpi_printers`.`entities_id`, `glpi_networkequipments`.`entities_id`, `glpi_peripherals`.`entities_id`) AS device_entities_id,
         CONCAT_WS('', `glpi_computers`.`comment`, `glpi_printers`.`comment`, `glpi_networkequipments`.`comment`, `glpi_peripherals`.`comment`) AS comment,
         ...
         `glpi_locations`.`comment` AS locationComment, CONCAT(`glpi_locations`.`latitude`, ',', `glpi_locations`.`longitude`) AS locationGPS,
         ...
         LEFT JOIN `glpi_peripherals`
            ON `glpi_plugin_monitoring_componentscatalogs_hosts`.`items_id` = `glpi_peripherals`.`id`
               AND `glpi_plugin_monitoring_componentscatalogs_hosts`.`itemtype`='Peripheral'
         ...
         LEFT JOIN `glpi_entities`
            ON ((`glpi_computers`.`entities_id` = `glpi_entities`.`id`
                ...
                OR
                (`glpi_peripherals`.`entities_id` = `glpi_entities`.`id`
                  AND `glpi_peripherals`.`id` IS NOT NULL)
                )

         LEFT JOIN `glpi_locations` ON `glpi_locations`.`id` = `glpi_computers`.`locations_id`
          OR `glpi_locations`.`id` = `glpi_printers`.`locations_id` 
          OR `glpi_locations`.`id` = `glpi_networkequipments`.`locations_id` 
          OR `glpi_locations`.`id` = `glpi_peripherals`.`locations_id`
        ...
        $dependency->deleteHostDependencies();
        ...
        while ($data = $DB->fetch_array($result)) {
            ...
            $a_hosts[$i] = $this->add_value_type(
                self::shinkenFilter($class->fields['name']), 'host_name', $a_hosts[$i]);
            ...
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - add host " . $a_hosts[$i]['host_name'] . "\n"
            );
            ...
            if (isset(self::$shinkenParameters['glpi']['entityId'])) {
                $a_hosts[$i] = $this->add_value_type(
                    $data['entityId'], self::$shinkenParameters['glpi']['entityId'], $a_hosts[$i]);
            }
            if (isset(self::$shinkenParameters['glpi']['itemType'])) {
                $a_hosts[$i] = $this->add_value_type(
                    $classname, self::$shinkenParameters['glpi']['itemType'], $a_hosts[$i]);
            }
            if (isset(self::$shinkenParameters['glpi']['itemId'])) {
                $a_hosts[$i] = $this->add_value_type(
                    $data['items_id'], self::$shinkenParameters['glpi']['itemId'], $a_hosts[$i]);
            }

            if (isset(self::$shinkenParameters['glpi']['entityName'])) {
                $a_hosts[$i] = $this->add_value_type(
                    strtolower(self::shinkenFilter($data['entityName'])), self::$shinkenParameters['glpi']['entityName'], $a_hosts[$i]);
            }
            ...
            if (isset(self::$shinkenParameters['glpi']['entityComplete'])) {
                $a_hosts[$i] = $this->add_value_type(
                    self::shinkenFilter($data['entityFullName']), self::$shinkenParameters['glpi']['entityComplete'], $a_hosts[$i]);
            }
            $data['entityFullName'] = preg_replace("/_/", ".", $data['entityFullName']);
            ...
            if (isset(self::$shinkenParameters['graphite']['prefix']['name'])) {
                ...
                $a_hosts[$i] = $this->add_value_type(
                    strtolower(self::$shinkenParameters['graphite']['prefix']['value'] . self::graphiteFilter($data['entityFullName'])), self::$shinkenParameters['graphite']['prefix']['name'], $a_hosts[$i]);
            }
            ...
            if (isset(self::$shinkenParameters['glpi']['location'])) {
                if (!empty($data['locationName'])) {
                    ...
                    $a_hosts[$i] = $this->add_value_type(
                        $string, self::$shinkenParameters['glpi']['location'], $a_hosts[$i]);
            ...
            if (isset(self::$shinkenParameters['glpi']['lat'])) {
                if (!empty($data['locationGPS'])) {
                    ...
                    if (count($split) > 2) {
                        ...
                        $a_hosts[$i] = $this->add_value_type(
                            $split[0], self::$shinkenParameters['glpi']['lat'], $a_hosts[$i]);
                        $a_hosts[$i] = $this->add_value_type(
                            $split[1], self::$shinkenParameters['glpi']['lng'], $a_hosts[$i]);
                        $a_hosts[$i] = $this->add_value_type(
                            $split[2], self::$shinkenParameters['glpi']['alt'], $a_hosts[$i]);
            ...
            // Hostgroup name
            $a_hosts[$i] = $this->add_value_type(
                preg_replace("/[ ]/", "_", self::shinkenFilter($data['entityName'])), 'hostgroups', $a_hosts[$i]);
            // Alias
            $a_hosts[$i] = $this->add_value_type(
                $this->shinkenFilter($data['entityName']) . " / " . $a_hosts[$i]['host_name'], 'alias', $a_hosts[$i]);
            ...
            if (isset(self::$shinkenParameters['webui']['hostIcons']['name'])) {
                $a_hosts[$i] = $this->add_value_type(
                    self::$shinkenParameters['webui']['hostIcons']['value'], self::$shinkenParameters['webui']['hostIcons']['name'], $a_hosts[$i]);
            }
            if (isset(self::$shinkenParameters['webui']['hostView']['name'])) {
                $a_hosts[$i] = $this->add_value_type(
                    self::$shinkenParameters['webui']['hostView']['value'], self::$shinkenParameters['webui']['hostView']['name'], $a_hosts[$i]);
            }
            ...
            if ($data['itemtype'] != 'NetworkEquipment') {
               ...
               AND `items_id`='" . $data['items_id'] . "'");
                foreach ($a_networkports as $data_n) {
                    ...
                    if ($networkports_id) {
                        ...
                        if ($networkPort->fields['itemtype'] == 'NetworkEquipment') {
                            ...
                            $dependency->addHostDependencyByDatas($data, $networkPort->fields);
                        }
                    }
                }
                ...
                if (empty($parent)) {
                    if ($default_host['parents'] == 'entity') {
                        foreach ($a_hosts[$i]['hostgroups'] as $val) {
                            $a_hosts[$i] = $this->add_value_type(
                                self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . $val, 'parents', $a_hosts[$i]);
                        }
                    } else {
                ...
                // dependecies NetworkEquipment-NetworkEquipment based on the network topology layers
                // having each layer one networkequipment type associated
                // possible layers are: access -(depends)-> distribution -(depends)-> core.
                $this->setNEDependenciesByLayer($data);
            }

            // Host check command
            $pmComponent->getFromDB($pmHostconfig->getValueAncestor('plugin_monitoring_components_id', $class->fields['entities_id'], $classname, $class->getID()));
                    ...
                    if (!isset($a_arguments[$arg])) {
                        ...
                        if (strstr($a_arguments[$arg], "[[HOSTNAME]]")) {
                            $a_arguments[$arg] = str_replace(
                                "[[HOSTNAME]]", $class->fields['name'], $a_arguments[$arg]);
            ...
            $a_hosts[$i] = $this->add_value_type(
                PluginMonitoringCommand::$command_prefix . $pmCommand->fields['command_name'] . $args, 'check_command', $a_hosts[$i]);

            // Check retries
            $pmCheck->getFromDB($a_fields['plugin_monitoring_checks_id']);
            $a_hosts[$i] = $this->add_value_type(
                $pmCheck->fields['check_interval'], 'check_interval', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                $pmCheck->fields['retry_interval'], 'retry_interval', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                $pmCheck->fields['max_check_attempts'], 'max_check_attempts', $a_hosts[$i]);

            $a_hosts[$i] = $this->add_value_type(
                $a_fields['active_checks_enabled'], 'active_checks_enabled', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                $a_fields['passive_checks_enabled'], 'passive_checks_enabled', $a_hosts[$i]);
            ...
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - add host " . $a_hosts[$i]['host_name'] . " in entity " . $data['entityId'] . ", calendar: " . $cid . "\n"
            );

            if ($calendar->getFromDB($cid) && $this->_addTimeperiod($data['entityId'], $cid)) {
                $a_hosts[$i] = $this->add_value_type(
                    self::shinkenFilter($calendar->fields['name'] . $timeperiodsuffix), 'check_period', $a_hosts[$i]);
            } else {
                $a_hosts[$i] = $this->add_value_type(
            ...
            if ($a_fields['freshness_count'] == 0) {
                ...
                $a_hosts[$i] = $this->add_value_type(
                    $a_fields['freshness_count'] * $multiple, 'freshness_threshold', $a_hosts[$i]);
            }
            ...
            if ($a_fields['plugin_monitoring_eventhandlers_id'] > 0) {
                if ($pmEventhandler->getFromDB($a_fields['plugin_monitoring_eventhandlers_id'])) {
                    $a_hosts[$i] = $this->add_value_type(
                        PluginMonitoringCommand::$command_prefix . $pmEventhandler->fields['command_name'], 'event_handler', $a_hosts[$i]);
                    $a_hosts[$i] = $this->add_value_type(
            ...
            if (isset($a_fields['business_priority'])) {
                $a_hosts[$i] = $this->add_value_type(
                    $a_fields['business_priority'], 'business_impact', $a_hosts[$i]);
            } else {
                if (!empty($default_host['business_impact'])) {
                    $a_hosts[$i] = $this->add_value_type(
                        $default_host['business_impact'], 'business_impact', $a_hosts[$i]);
                } else {
                    $a_hosts[$i] = $this->add_value_type(
            ...
            if ($DB->numrows($resultcont) != 0) {
                ...
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', "generateHostsCfg - CC, host: {$a_hosts[$i]['host_name']} in {$a_componentscatalogs_hosts['plugin_monitoring_componentscalalog_id']}\n"
                );
                ...
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', "generateHostsCfg - CC, host: {$a_hosts[$i]['host_name']} in {$pmComponentscatalog->fields['name']}\n"
                );
                // Host template/tag is CC name
                $a_hosts[$i] = $this->add_value_type(
                    $pmComponentscatalog->fields['name'], 'use', $a_hosts[$i]);

                // Hosts notification
                $pmHNTemplate = new PluginMonitoringHostnotificationtemplate();
                if ((!isset($pmComponentscatalog->fields['hostsnotification_id'])) || (!$pmHNTemplate->getFromDB($pmComponentscatalog->fields['hostsnotification_id']))) {
                    ...
                    PluginMonitoringToolbox::logIfExtradebug(
                        'pm-shinken', "generateHostsCfg - CC, host: {$a_hosts[$i]['host_name']} in {$pmComponentscatalog->fields['name']}, no notifications.\n"
                    );
                } else {
                    $a_HN = $pmHNTemplate->fields;

                    PluginMonitoringToolbox::logIfExtradebug(
                        'pm-shinken', "generateHostsCfg - CC, host: {$a_hosts[$i]['host_name']} in {$pmComponentscatalog->fields['name']}, notification template: {$a_HN['name']}.\n"
                    );
                    ...
                    if ($a_HN['host_notifications_enabled'] == 0) {
                        ...
                        PluginMonitoringToolbox::logIfExtradebug(
                            'pm-shinken', "generateHostsCfg - CC, host: {$a_hosts[$i]['host_name']} in {$pmComponentscatalog->fields['name']}, no notifications for host.\n"
                        );
                    } else {
                        if (!isset($a_HN['host_notification_period']) ||
                            ...
                            PluginMonitoringToolbox::logIfExtradebug(
                                'pm-shinken', "generateHostsCfg - CC, host: {$a_hosts[$i]['host_name']}, host notification period : {$a_HN['host_notification_period']}.\n"
                            );
                            // Notification period
                            if ($calendar->getFromDB($a_HN['host_notification_period']) && $this->_addTimeperiod($class->fields['entities_id'], $a_HN['host_notification_period'])) {
                                $a_hosts[$i] = $this->add_value_type(
                                    self::shinkenFilter($calendar->fields['name'] . $timeperiodsuffix), 'notification_period', $a_hosts[$i]);
                            } else {
                                if (!empty($default_host['notification_period']))
                                    $a_hosts[$i] = $this->add_value_type(
                                        $default_host['notification_period'], 'notification_period', $a_hosts[$i]);
                            }
                            ...
                            if (isset($pmComponentscatalog->fields['notification_interval'])) {
                                $a_hosts[$i] = $this->add_value_type(
                                    $pmComponentscatalog->fields['notification_interval'], 'notification_interval', $a_hosts[$i]);
                            } else {
                                if (!empty($default_host['notification_interval']))
                                    $a_hosts[$i] = $this->add_value_type(
                                        $default_host['notification_interval'], 'notification_interval', $a_hosts[$i]);
                            }
                        }
                    }
                }
               ...
               AND `items_id`='" . $a_componentscatalogs_hosts['plugin_monitoring_componentscalalog_id'] . "'");
                    ...
                    if ($data_contact['users_id'] > 0) {
                        if (isset($a_contacts_entities[$a_componentscatalogs_hosts['plugin_monitoring_componentscalalog_id']][$data_contact['users_id']])) {
                            if (in_array($class->fields['entities_id'], $a_contacts_entities[$a_componentscatalogs_hosts['plugin_monitoring_componentscalalog_id']][$data_contact['users_id']])) {
                                $user->getFromDB($data_contact['users_id']);
                                $a_hosts[$i] = $this->add_value_type(
                                    $user->fields['name'], 'contacts', $a_hosts[$i]);
                            }
                        }
                    } else if ($data_contact['groups_id'] > 0) {
                        $queryg = "SELECT * FROM `glpi_groups_users` WHERE `groups_id`='" . $data_contact['groups_id'] . "'";
                        $resultg = $DB->query($queryg);
                        while ($datag = $DB->fetch_array($resultg)) {
                            if (isset($a_contacts_entities[$a_componentscatalogs_hosts['plugin_monitoring_componentscalalog_id']][$datag['users_id']])) {
                                if (in_array($class->fields['entities_id'], $a_contacts_entities[$a_componentscatalogs_hosts['plugin_monitoring_componentscalalog_id']][$datag['users_id']])) {
                ...
                if (count($a_hosts[$i]['contacts']) == 0) {
                    if (self::$shinkenParameters['shinken']['fake_contacts']['build']) {
                        $a_hosts[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['fake_contacts']['contact_name'], 'contacts', $a_hosts[$i]);
                    } else {
                        $a_hosts[$i] = $this->add_value_type(
                            '', 'contacts', $a_hosts[$i]);
                    }
        ...
        if (count($a_listBA) > 0) {
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - add host_for_bp\n"
            );
            $a_hosts[$i] = array();
            $a_hosts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . self::$shinkenParameters['shinken']['fake_bp_hosts']['hostname'], 'host_name', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                PluginMonitoringCommand::$command_prefix . self::$shinkenParameters['shinken']['fake_hosts']['check_command'], 'check_command', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                'Fake host for business rules', 'alias', $a_hosts[$i]);
            // $a_hosts[$i]['_HOSTID'] = '0';
            // $a_hosts[$i]['_ITEMSID'] = '0';
            // $a_hosts[$i]['_ITEMTYPE'] = 'Computer';
            $a_hosts[$i] = $this->add_value_type('127.0.0.1', 'address', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . self::$shinkenParameters['shinken']['fake_hosts']['root_parent'], 'parents', $a_hosts[$i]);
            $a_hosts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['fake_hosts']['hostgroup_name'], 'hostgroups', $a_hosts[$i]);
            ...
            $a_hosts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['fake_hosts']['check_period'], 'check_period', $a_hosts[$i]);
            // Host entity jetlag ...
            ...
            foreach ($a_calendars as $calendar) {
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', " - add host_for_bp 2: " . serialize($calendar) . " / " . $calendar['name'] . "\n"
                );
                if ($this->_addTimeperiod(0, $calendar['id'])) {
                    $a_hosts[$i] = $this->add_value_type(
                        self::shinkenFilter($calendar['name'] . $timeperiodsuffix), 'check_period', $a_hosts[$i]);
                }
            }
            ...
            if (!empty(self::$shinkenParameters['shinken']['fake_hosts']['use'])) {
                $a_hosts[$i] = $this->add_value_type(
                    self::$shinkenParameters['shinken']['fake_hosts']['use'], 'use', $a_hosts[$i]);
            }
        ...
        if (self::$shinkenParameters['shinken']['fake_hosts']['build']) {
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - add fake hosts for parents relationship\n"
            );

            // Main root parent
            $a_hosts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . self::$shinkenParameters['shinken']['fake_hosts']['root_parent'], 'host_name', $a_hosts[$i]);
            ...
            while ($dataEntity = $DB->fetch_array($result)) {
                ...
                $a_hosts[$i] = $this->add_value_type(
                    self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . $fake_host_name, 'host_name', $a_hosts[$i]);
                $a_hosts[$i] = $this->add_value_type(
                    PluginMonitoringCommand::$command_prefix . self::$shinkenParameters['shinken']['fake_hosts']['check_command'], 'check_command', $a_hosts[$i]);
                $a_hosts[$i] = $this->add_value_type(
            ...
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', "End generateHostgroupsCfg\n"
            );
        }
        ...
        foreach ($a_parents_found as $host => $num) {
            if (!isset($a_hosts_found[$host])) {
                ...
                foreach ($a_hosts as $id => $data) {
                    if (isset($data['parents']) && $data['parents'] == $host) {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateHostsCfg\n"
        );
    ...
    function generateServicesCfg($file = 0, $tag = '') {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateServicesCfg services ($tag) ...\n"
        );
        ...
        while ($data = $DB->fetch_array($result)) {
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - fetching service: {$data['id']}\n"
            );
            ...
            if (empty($a_component)) {
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', " *** fetching service: {$data['id']} - no associated component !\n"
                );
                continue;
            }
            $a_hostname = array();
            ...
            if (isset($componentscatalog_hosts[$data['plugin_monitoring_componentscatalogs_hosts_id']])) {
                ...
                if (!$item->getFromDB($datah['items_id'])) {
                    PluginMonitoringToolbox::logIfExtradebug(
                        'pm-shinken', " *** fetching service: {$data['id']} - no itemtype/items_id found !\n"
                    );
                    continue;
                }
                ...
                if (empty($item->fields['name'])) {
                    PluginMonitoringToolbox::logIfExtradebug(
                        'pm-shinken', " *** fetching service: {$data['id']} - no item name found !\n"
                    );
                    continue;
                }
                ...
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', " - fetching service: {$data['id']} ({$a_component['name']} - {$a_component['id']}) - {$datah['itemtype']} - {$datah['items_id']} -> {$item->fields['name']}\n"
                );
            ...
            if (count($a_hostname) > 0) {
                if (isset($_SESSION['plugin_monitoring']['servicetemplates'][$a_component['id']])) {
                    $a_services[$i] = $this->add_value_type(
                        $_SESSION['plugin_monitoring']['servicetemplates'][$a_component['id']], 'use', $a_services[$i]);
                }
                $a_services[$i] = $this->add_value_type(
                    implode(",", array_unique($a_hostname)), 'host_name', $a_services[$i]);
                $a_services[$i] = $this->add_value_type(
                    implode(",", array_unique($a_hostname_id)), '_HOSTITEMSID', $a_services[$i]);
                $a_services[$i] = $this->add_value_type(
                    implode(",", array_unique($a_hostname_type)), '_HOSTITEMTYPE', $a_services[$i]);

                // Define display_name / service_description
                if (!empty($a_component['description'])) {
                    $a_services[$i] = $this->add_value_type(
                        $a_component['description'], 'service_description', $a_services[$i]);
                } else {
                    $a_services[$i] = $this->add_value_type(
                        self::shinkenFilter($a_component['name']), 'service_description', $a_services[$i]);
                }
                ...
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', " - add service " . $a_services[$i]['service_description'] . " on " . $a_services[$i]['host_name'] . "\n"
                );

                if (isset(self::$shinkenParameters['glpi']['entityId'])) {
                    $a_services[$i] = $this->add_value_type(
                        $item->fields['entities_id'], self::$shinkenParameters['glpi']['entityId'], $a_services[$i]);
                }
                if (isset(self::$shinkenParameters['glpi']['itemType'])) {
                    $a_services[$i] = $this->add_value_type(
                        'Service', self::$shinkenParameters['glpi']['itemType'], $a_services[$i]);
                }
                if (isset(self::$shinkenParameters['glpi']['itemId'])) {
                    $a_services[$i] = $this->add_value_type(
                        $data['id'], self::$shinkenParameters['glpi']['itemId'], $a_services[$i]);
                }
                            ...
                            if (strstr($a_arguments[$arg], "[[HOSTNAME]]")) {
                                ...
                                if (isset($data['networkports_id']) && $data['networkports_id'] > 0) {
                                    ...
                                    $networkPort = PluginMonitoringHostaddress::getNetworkport(
                                            $a_services[$i]['_HOSTITEMSID'], $a_services[$i]['_HOSTITEMTYPE']);
                                }
                                if ($networkPort->getID() > 0) {
                                    $logicalnum = $networkPort->fields['logical_number'];
                                    $a_arguments[$arg] = str_replace("[[NETWORKPORTNUM]]", $logicalnum, $a_arguments[$arg]);
...
//                                 $ip = PluginMonitoringHostaddress::getIp($host['computers_id'], 'Computer', '');
                        ...
                        $a_services[$i] = $this->add_value_type(
                            PluginMonitoringCommand::$command_prefix . "check_nrpe!" . $alias_command, 'check_command', $a_services[$i]);
                    } else {
                        $a_services[$i] = $this->add_value_type(
                            PluginMonitoringCommand::$command_prefix . "check_nrpe!" . $a_command['command_name'], 'check_command', $a_services[$i]);
                    }
                } else {
                    $a_services[$i] = $this->add_value_type(
                        PluginMonitoringCommand::$command_prefix . $a_command['command_name'] . $args, 'check_command', $a_services[$i]);
                }
                ...
                if ($a_component['plugin_monitoring_eventhandlers_id'] > 0) {
                    if ($pmEventhandler->getFromDB($a_component['plugin_monitoring_eventhandlers_id'])) {
                        $a_services[$i] = $this->add_value_type(
                            PluginMonitoringCommand::$command_prefix . $pmEventhandler->fields['command_name'], 'event_handler', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type('1', 'event_handler_enabled', $a_services[$i]);
                    } else {
                        $a_services[$i] = $this->add_value_type('0', 'event_handler_enabled', $a_services[$i]);
                    }
                } else {
                    $a_services[$i] = $this->add_value_type('0', 'event_handler_enabled', $a_services[$i]);
                }
               ...
               AND `items_id`='" . $plugin_monitoring_componentscatalogs_id . "'");
                    ...
                    if ($data_contact['users_id'] > 0) {
                        if (isset($a_contacts_entities[$plugin_monitoring_componentscatalogs_id][$data_contact['users_id']])) {
                            if (in_array($data['entities_id'], $a_contacts_entities[$plugin_monitoring_componentscatalogs_id][$data_contact['users_id']])) {
                                ...
                                $a_services[$i] = $this->add_value_type(
                                    $a_users[$data_contact['users_id']], 'contacts', $a_services[$i]);
                            }
                        }
                    } else if ($data_contact['groups_id'] > 0) {
                        $queryg = "SELECT * FROM `glpi_groups_users`
                     WHERE `groups_id`='" . $data_contact['groups_id'] . "'";
                        ...
                        while ($datag = $DB->fetch_array($resultg)) {
                            if (in_array($data['entities_id'], $a_contacts_entities[$plugin_monitoring_componentscatalogs_id][$datag['users_id']])) {
                                ...
                                $a_services[$i] = $this->add_value_type(
                                    $a_users[$datag['users_id']], 'contacts', $a_services[$i]);
                            }
                        }
                    }
                }
                ...
                if (count($a_services[$i]['contacts']) == 0) {
                    if (self::$shinkenParameters['shinken']['fake_contacts']['build']) {
                        $a_services[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['fake_contacts']['contact_name'], 'contacts', $a_services[$i]);
                    } else {
                ...
                if (!isset($_SESSION['plugin_monitoring']['servicetemplates'][$a_component['id']])) {
                    ...
                    $a_services[$i] = $this->add_value_type(
                        $a_check['check_interval'], 'check_interval', $a_services[$i]);
                    $a_services[$i] = $this->add_value_type(
                        $a_check['retry_interval'], 'retry_interval', $a_services[$i]);
                    $a_services[$i] = $this->add_value_type(
                        $a_check['max_check_attempts'], 'max_check_attempts', $a_services[$i]);
                    if (isset($a_calendars[$a_component['calendars_id']]) && $this->_addTimeperiod($entities_id, $a_component['calendars_id'])) {
                        $a_services[$i] = $this->add_value_type(
                            self::shinkenFilter($a_calendars[$a_component['calendars_id']]['name'] . $timeperiodsuffix), 'check_period', $a_services[$i]);
                    } else {
                        $a_services[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['services']['check_period'], 'check_period', $a_services[$i]);
                    }
                    $elements = array(
                    ...
                    if (!empty(self::$shinkenParameters['shinken']['services']['business_impact'])) {
                        $a_services[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['services']['business_impact'], 'business_impact', $a_services[$i]);
                    } else {
                        $a_services[$i] = $this->add_value_type(
                    ...
                    PluginMonitoringToolbox::logIfExtradebug(
                        'pm-shinken', "generateServicesCfg - CC, service: {$a_services[$i]['service_description']}/{$a_services[$i]['host_name']} in {$a_componentscatalog['name']}\n"
                    );
                    $pmSNTemplate = new PluginMonitoringServicenotificationtemplate();
                    if ((!isset($a_componentscatalog['servicesnotification_id'])) || (!$pmSNTemplate->getFromDB($a_componentscatalog['servicesnotification_id']))) {
                        ...
                        PluginMonitoringToolbox::logIfExtradebug(
                            'pm-shinken', "generateServicesCfg - CC, service: {$a_services[$i]['service_description']}/{$a_services[$i]['host_name']} in {$a_componentscatalog['name']}, no notifications.\n"
                        );
                    } else {
                        $a_SN = $pmSNTemplate->fields;

                        PluginMonitoringToolbox::logIfExtradebug(
                            'pm-shinken', "generateServicesCfg - CC, service: {$a_services[$i]['service_description']}/{$a_services[$i]['host_name']} in {$a_componentscatalog['name']}, notification template: {$a_SN['name']}.\n"
                        );
                        ...
                        if ($a_SN['service_notifications_enabled'] == 0) {
                            ...
                            if (!isset($a_SN['service_notification_period']) ||
                                ...
                                if (isset($a_calendars[$a_SN['service_notification_period']]) && $this->_addTimeperiod($entities_id, $a_SN['service_notification_period'])) {
                                    $a_services[$i] = $this->add_value_type(
                                        self::shinkenFilter($a_calendars[$a_SN['service_notification_period']]['name'] . $timeperiodsuffix), 'notification_period', $a_services[$i]);
                                } else {
                                    if (!empty(self::$shinkenParameters['shinken']['services']['notification_period']))
                                        $a_services[$i] = $this->add_value_type(
                                            self::$shinkenParameters['shinken']['services']['notification_period'], 'notification_period', $a_services[$i]);
                                }
                                ...
                                if (isset($a_componentscatalog['notification_interval'])) {
                                    $a_services[$i] = $this->add_value_type(
                                        $a_componentscatalog['notification_interval'], 'notification_interval', $a_services[$i]);
                                } else {
                                    if (!empty(self::$shinkenParameters['shinken']['hosts']['notification_interval'])) {
                                        $a_services[$i] = $this->add_value_type(
                                            self::$shinkenParameters['shinken']['hosts']['notification_interval'], 'notification_interval', $a_services[$i]);
                                    }
                                }
                            }
                        }
                    ...
                    if ($calendar->getFromDB($cid) && $this->_addTimeperiod($item->fields['entities_id'], $cid)) {
                        $a_services[$i] = $this->add_value_type(
                            self::shinkenFilter($calendar->fields['name'] . $timeperiodsuffix), 'check_period', $a_services[$i]);
                    }
                    ...
                    if (self::$shinkenParameters['shinken']['services']['check_period'] != '') {
                        ...
                        if (isset($a_componentscatalog['calendars_id'])) {
                            if (isset($a_calendars[$a_componentscatalog['calendars_id']]) && $this->_addTimeperiod($entities_id, $a_componentscatalog['calendars_id'])) {
                                $a_services[$i] = $this->add_value_type(
                                    self::shinkenFilter($a_calendars[$a_componentscatalog['calendars_id']]['name'] . $timeperiodsuffix), 'check_period', $a_services[$i]);
                            } else {
                                $a_services[$i] = $this->add_value_type(
                                    self::$shinkenParameters['shinken']['services']['check_period'], 'check_period', $a_services[$i]);
                            }
                ...
                if (isset(self::$shinkenParameters['webui']['serviceIcons']['name'])) {
                    $a_services[$i] = $this->add_value_type(
                        self::$shinkenParameters['webui']['serviceIcons']['value'], self::$shinkenParameters['webui']['serviceIcons']['name'], $a_services[$i]);
                }
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateServicesCfg services\n"
        );

        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateServicesCfg business rules ...\n"
        );
            ...
            if (isset($a_entities_allowed['-1'])
                ...
                foreach ($a_grouplist as $gdata) {
                    ...
                    foreach ($a_listBR as $dataBR) {
                        ...
                        PluginMonitoringToolbox::logIfExtradebug(
                            'pm-shinken', "   - SC group : " . $a_group[$gdata['id']] . "\n"
                        );
                    }
                }
                if (count($a_group) > 0) {
                    if (isset($a_checks[$dataBA['plugin_monitoring_checks_id']])) {
                        ...
                        if (isset($a_calendars[$dataBA['calendars_id']])) {
                            $a_services[$i] = $this->add_value_type(
                                $a_calendars[$dataBA['calendars_id']]['name'], 'check_period', $a_services[$i]);
                        }
                        $a_services[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . self::$shinkenParameters['shinken']['fake_bp_hosts']['hostname'], 'host_name', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                            $dataBA['business_priority'], 'business_impact', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                            self::shinkenFilter($dataBA['name']), 'service_description', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                        ...
                        $a_services[$i] = $this->add_value_type(
                            $command . implode("&", $a_group), 'check_command', $a_services[$i]);
                        if ($dataBA['notification_interval'] != '30') {
                            $a_services[$i] = $this->add_value_type(
                                $dataBA['notification_interval'], 'notification_interval', $a_services[$i]);
                        } else {
                            $a_services[$i] = $this->add_value_type(
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateServicesCfg business rules\n"
        );

        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateServicesCfg business rules templates ...\n"
        );
        ...
        foreach ($a_listBA as $dataBA) {
            ...
            if (isset($a_entities_allowed['-1'])
                ...
                foreach ($a_entitiesServices as $idEntity => $a_entityServices) {
                    ...
                    PluginMonitoringToolbox::logIfExtradebug(
                        'pm-shinken', "   - SC templated services for an entity : " . $idEntity . "\n"
                    );
                    ...
                    foreach ($a_derivatedSC as $a_derivated) {
                        PluginMonitoringToolbox::logIfExtradebug(
                            'pm-shinken', "   - a_derivated : " . $a_derivated['name'] . "\n"
                        );
                    ...
                    if (count($a_group) > 0) {
                        ...
                        if (isset($a_calendars[$a_derivatedSC['calendars_id']])) {
                            $a_services[$i] = $this->add_value_type(
                                $a_calendars[$a_derivatedSC['calendars_id']]['name'], 'check_period', $a_services[$i]);
                        }
                        $a_services[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['fake_hosts']['name_prefix'] . self::$shinkenParameters['shinken']['fake_hosts']['bp_host'], 'host_name', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                            $a_derivatedSC['business_priority'], 'business_impact', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                            self::shinkenFilter($a_derivatedSC['name']), 'service_description', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                            $a_derivatedSC['entities_id'], '_ENTITIESID', $a_services[$i]);
                        $a_services[$i] = $this->add_value_type(
                        ...
                        if ($a_derivatedSC['notification_interval'] != 30) {
                            $a_services[$i] = $this->add_value_type(
                                $a_derivatedSC['notification_interval'], 'notification_interval', $a_services[$i]);
                        } else {
                            $a_services[$i] = $this->add_value_type(
                                '30', 'notification_interval', $a_services[$i]);
                        }
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateServicesCfg business rules templates\n"
        );
    ...
    function generateHostDependenciesCfg($file = 0, $tag = '') {
        global $DB;

        $a_hostpendency = array();

        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Start generateHostDependenciesCfg\n"
        );
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateHostDependenciesCfg\n"
        );

        if ($file == "1") {
            $config = "# Generated by plugin monitoring for GLPI\n# on " . date("Y-m-d H:i:s") . "\n\n";

            foreach ($a_hostpendency as $data) {
                $config .= $this->writeFile("servicedependency", $data);
            }
            return array('hostdependency.cfg', $config);
        } else {
            return $a_hostpendency;
        }
    }

    function generateServiceDependenciesCfg($file = 0, $tag = '') {
        global $DB;

        $a_servicependency = array();

        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Start generateServiceDependenciesCfg\n"
        );
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateServiceDependenciesCfg\n"
        );

        if ($file == "1") {
            $config = "# Generated by plugin monitoring for GLPI\n# on " . date("Y-m-d H:i:s") . "\n\n";

            foreach ($a_servicependency as $data) {
                $config .= $this->writeFile("servicedependency", $data);
            }
            return array('servicedependency.cfg', $config);
        } else {
            return $a_servicependency;
        }
    }
    ...
    function generateTemplatesCfg($file = 0, $tag = '') {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateTemplatesCfg ($tag) ...\n"
        );
        $pMonitoringCheck = new PluginMonitoringCheck();
        ...
        if ($DB->numrows($result) != 0) {
            while ($data = $DB->fetch_array($result)) {
                ...
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', " - add template " . 'template' . $data['id'] . '-service' . "\n"
                );
                // Fix service template association bug: #191
                ...
                $a_servicetemplates[$i] = $this->add_value_type(
                    self::shinkenFilter($data['name']), 'name', $a_servicetemplates[$i]);
                // Alias is not used by Shinken !
                $a_servicetemplates[$i] = $this->add_value_type(
                    $data['description'] . ' / ' . $data['name'], 'alias', $a_servicetemplates[$i]);
                if (isset($data['business_priority'])) {
                    $a_servicetemplates[$i] = $this->add_value_type(
                        $data['business_priority'], 'business_impact', $a_servicetemplates[$i]);
                } else {
                    if (!empty(self::$shinkenParameters['shinken']['services']['business_impact'])) {
                        $a_servicetemplates[$i] = $this->add_value_type(
                            self::$shinkenParameters['shinken']['services']['business_impact'], 'business_impact', $a_servicetemplates[$i]);
                    } else {
                        $a_servicetemplates[$i] = $this->add_value_type(
                ...
                $a_servicetemplates[$i] = $this->add_value_type(
                    $pMonitoringCheck->fields['check_interval'], 'check_interval', $a_servicetemplates[$i]);
                $a_servicetemplates[$i] = $this->add_value_type(
                    $pMonitoringCheck->fields['retry_interval'], 'retry_interval', $a_servicetemplates[$i]);
                $a_servicetemplates[$i] = $this->add_value_type(
                    $pMonitoringCheck->fields['max_check_attempts'], 'max_check_attempts', $a_servicetemplates[$i]);
                ...
                $a_servicetemplates[$i] = $this->add_value_type(
                    $data['active_checks_enabled'], 'active_checks_enabled', $a_servicetemplates[$i]);
                $a_servicetemplates[$i] = $this->add_value_type(
                    $data['passive_checks_enabled'], 'passive_checks_enabled', $a_servicetemplates[$i]);
                $a_servicetemplates[$i] = $this->add_value_type(
                ...
                if ($data['freshness_count'] == 0) {
                    ...
                    $a_servicetemplates[$i] = $this->add_value_type(
                        ($data['freshness_count'] * $multiple), 'freshness_threshold', $a_servicetemplates[$i]);
                }
                $a_servicetemplates[$i] = $this->add_value_type(
                ...
                $a_servicetemplates[$i] = $this->add_value_type(
                    self::$shinkenParameters['shinken']['services']['stalking_options'], 'stalking_options', $a_servicetemplates[$i]);

                if (isset(self::$shinkenParameters['shinken']['services']['flap_detection_enabled'])) {
                    $a_servicetemplates[$i] = $this->add_value_type(
                        self::$shinkenParameters['shinken']['services']['flap_detection_enabled'], 'flap_detection_enabled', $a_servicetemplates[$i]);
                    $a_servicetemplates[$i] = $this->add_value_type(
                        self::$shinkenParameters['shinken']['services']['flap_detection_options'], 'flap_detection_options', $a_servicetemplates[$i]);
                    $a_servicetemplates[$i] = $this->add_value_type(
                        self::$shinkenParameters['shinken']['services']['low_flap_threshold'], 'low_flap_threshold', $a_servicetemplates[$i]);
                    $a_servicetemplates[$i] = $this->add_value_type(
                        self::$shinkenParameters['shinken']['services']['high_flap_threshold'], 'high_flap_threshold', $a_servicetemplates[$i]);
                }
                $a_servicetemplates[$i] = $this->add_value_type(
                    self::$shinkenParameters['shinken']['services']['failure_prediction_enabled'], 'failure_prediction_enabled', $a_servicetemplates[$i]);
                $a_servicetemplates[$i] = $this->add_value_type(
                    self::$shinkenParameters['shinken']['services']['retain_status_information'], 'retain_status_information', $a_servicetemplates[$i]);
                $a_servicetemplates[$i] = $this->add_value_type(
                    self::$shinkenParameters['shinken']['services']['retain_nonstatus_information'], 'retain_nonstatus_information', $a_servicetemplates[$i]);
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateTemplatesCfg\n"
        );
    ...
    function generateHostgroupsCfg($file = 0, $tag = '') {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateHostgroupsCfg ($tag) ...\n"
        );
        $pmEntity = new PluginMonitoringEntity();
        ...
        while ($data = $DB->fetch_array($result)) {
            ...
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - add group $hostgroup_name ...\n"
            );
            ...
            if (count($a_sons_list) > 1) {
                ...
                foreach ($a_sons_list as $son_entity) {
                    if ($son_entity == $data['entityId'])
                        continue;
                    if (!in_array($son_entity, $a_entities_list))
                        continue;

                    $pmEntity = new Entity();
                    $pmEntity->getFromDB($son_entity);
                    // Only immediate sub level are considered as hostgroup members
                    if ($data['entityLevel'] + 1 != $pmEntity->fields['level'])
                        continue;
    ...
    function generateContactsCfg($file = 0, $tag = '') {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateContactsCfg ($tag) ...\n"
        );
...
//      );
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateContactsCfg\n"
        );
    ...
    function _addContactUser($a_contacts, $users_id, $i) {
        ...
        if (empty($a_pmcontact) OR ( isset($a_pmcontact['plugin_monitoring_contacttemplates_id'])
        ...
        if (isset(self::$shinkenParameters['shinken']['contacts']['note'])) {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['contacts']['note'] . $a_pmcontact['name'], 'note', $a_contacts[$i]);
        }
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "- generate contact '" . $user->fields['name'] . "' - " . $user->getName() . "\n"
        );
        ...
        $a_contacts[$i] = $this->add_value_type(
            $a_pmcontact['host_notifications_enabled'], 'host_notifications_enabled', $a_contacts[$i]);
        $a_contacts[$i] = $this->add_value_type(
            $a_pmcontact['service_notifications_enabled'], 'service_notifications_enabled', $a_contacts[$i]);
        ...
        if ($calendar->getFromDB($a_pmcontact['service_notification_period']) && $this->_addTimeperiod($user->fields['entities_id'], $a_pmcontact['service_notification_period'])) {
            $a_contacts[$i] = $this->add_value_type(
                self::shinkenFilter($calendar->fields['name'] . $timeperiodsuffix), 'service_notification_period', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['contacts']['service_notification_period'], 'service_notification_period', $a_contacts[$i]);
        }
        ...
        if ($calendar->getFromDB($a_pmcontact['host_notification_period']) && $this->_addTimeperiod($user->fields['entities_id'], $a_pmcontact['host_notification_period'])) {
            $a_contacts[$i] = $this->add_value_type(
                self::shinkenFilter($calendar->fields['name'] . $timeperiodsuffix), 'host_notification_period', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['contacts']['host_notification_period'], 'host_notification_period', $a_contacts[$i]);
        }
        ...
        if (isset($pmNotificationcommand->fields['command_name'])) {
            $a_contacts[$i] = $this->add_value_type(
                PluginMonitoringCommand::$command_prefix . $pmNotificationcommand->fields['command_name'], 'service_notification_commands', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
        ...
        if (isset($pmNotificationcommand->fields['command_name'])) {
            $a_contacts[$i] = $this->add_value_type(
                PluginMonitoringCommand::$command_prefix . $pmNotificationcommand->fields['command_name'], 'host_notification_commands', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
        ...
        if (isset(self::$shinkenParameters['shinken']['contacts']['retain_status_information'])) {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['contacts']['retain_status_information'], 'retain_status_information', $a_contacts[$i]);
        }
        if (isset(self::$shinkenParameters['shinken']['contacts']['retain_nonstatus_information'])) {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['shinken']['contacts']['retain_nonstatus_information'], 'retain_nonstatus_information', $a_contacts[$i]);
        }
        if (isset($a_pmcontact['shinken_administrator'])) {
            $a_contacts[$i] = $this->add_value_type(
                $a_pmcontact['shinken_administrator'], 'is_admin', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['webui']['contacts']['is_admin'], 'is_admin', $a_contacts[$i]);
        }
        if (isset($a_pmcontact['shinken_can_submit_commands'])) {
            $a_contacts[$i] = $this->add_value_type(
                $a_pmcontact['shinken_can_submit_commands'], 'can_submit_commands', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['webui']['contacts']['can_submit_commands'], 'can_submit_commands', $a_contacts[$i]);
        }
        if (empty($user->fields['password'])) {
            $a_contacts[$i] = $this->add_value_type(
                self::$shinkenParameters['webui']['contacts']['password'], 'password', $a_contacts[$i]);
        } else {
            $a_contacts[$i] = $this->add_value_type(
    ...
    function generateTimeperiodsCfg($file = 0, $tag = '') {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting generateTimeperiodsCfg ...\n"
        );

        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End generateTimeperiodsCfg\n"
        );
    ...
    function _addHoliday($entities_id = -1, $holidays_id = -1) {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting _addHoliday: $entities_id / $holidays_id ...\n"
        );
        $calendar = new Calendar();
        ...
        if (!isset($_SESSION['plugin_monitoring']['jetlag'][$entities_id])) {
            $_SESSION['plugin_monitoring']['jetlag'][$entities_id] = $hostconfig->getValueAncestor('jetlag', $entities_id);
        }
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', " - entity: $entities_id, jetlag: $timeperiodsuffix\n"
        );

        if (!$holiday->getFromDB($holidays_id)) {
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - invalid holiday: $holidays_id ...\n"
            );
            return false;
        }
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', " - holiday: $holidays_id, jetlag: $timeperiodsuffix\n"
        );
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', " - _addHoliday, building TP '{$tmp['timeperiod_name']}' for entity: $entities_id\n"
        );
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End _addHoliday: {$tmp['timeperiod_name']}\n"
        );
    ...
    function _addTimeperiod($entities_id = -1, $calendars_id = -1) {
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "Starting _addTimeperiod: $entities_id / $calendars_id ...\n"
        );
        $calendar = new Calendar();
        $calendarSegment = new CalendarSegment();
        $calendar_Holiday = new Calendar_Holiday();
        ...
        if (!$entity->getFromDB($entities_id)) {
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - invalid entity: $entities_id\n"
            );
            return false;
        }
        // Jetlag for required entity ...
        ...
        if (!isset($_SESSION['plugin_monitoring']['timeperiodsmapping'][$calendars_id])) {
            if (!$calendar->getFromDB($calendars_id)) {
                PluginMonitoringToolbox::logIfExtradebug(
                    'pm-shinken', " - invalid calendar: $calendars_id ...\n"
                );
                return false;
            }
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', " - _addTimeperiod, entity: $entities_id, jetlag: $timeperiodsuffix\n"
        );
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', " - _addTimeperiod, building calendar '{$tmp['timeperiod_name']}' for entity: $entities_id\n"
        );
        ...
        foreach ($a_cholidays as $a_choliday) {
            PluginMonitoringToolbox::logIfExtradebug(
                'pm-shinken', " - _addTimeperiod, building holiday '{$a_choliday['holidays_id']}'\n"
            );
...
//                     $tmp[$beforeday] = implode(',', $splitBeforeDay);
        ...
        PluginMonitoringToolbox::logIfExtradebug(
            'pm-shinken', "End _addTimeperiod: {$tmp['timeperiod_name']}\n"
        );
    ...
    /**
     * Sets the networkequipments dependencies based on topology layers.
     * @param array $data Needs: items_id and itemtype fields
     */
    private function setNEDependenciesByLayer($data) {
        $dependency = new PluginMonitoringDependency();
        $pmConfig = new PluginMonitoringConfig();
        $pmConfig->getFromDB(1);
        ...
        $type_id = $networkEquipment->fields['networkequipmenttypes_id'];

        $networkPort = new NetworkPort();
        $a_networkports = $networkPort->find("`itemtype`='" . $data['itemtype'] . "'
                  AND `items_id`='" . $data['items_id'] . "'");
        
        $ne_conected_to_me = array();
        $who_depends_i = array();
        ...
        foreach ($a_networkports as $data_n) {
            ...
            if ($networkports_id) {
                ...
                if ($networkPort->fields['itemtype'] == 'NetworkEquipment') {
                    ...
                    // network topology levels based on networkequipment type
                    switch ($networkEquipment->fields['networkequipmenttypes_id']) {
                        case $type_id:
                        case $pmConfig->fields['type_access_layer']:
                            // is the same layer or the lowest of the layers
                            $ne_conected_to_me[] = $networkEquipment->fields;
                            break;

                        case $pmConfig->fields['type_distribution_layer']:
                            // if layer is lower
                            if ($type_id == $pmConfig->fields['type_access_layer']
                                and $type_id != $pmConfig->fields['type_core_layer']) {
                                $dependency->addHostDependencyByDatas($data, $networkPort->fields);
                                $who_depends_i[] = $networkPort->fields;
                            }
                            break;

                        case $pmConfig->fields['type_core_layer']:
                            // if layer is lower
                            if ($type_id == $pmConfig->fields['type_access_layer']
                                or $type_id == $pmConfig->fields['type_distribution_layer']) {
                                $dependency->addHostDependencyByDatas($data, $networkPort->fields);
                                $who_depends_i[] = $networkPort->fields;
                            }
                            break;
        ...
        // search for another NE conected to this
        foreach ($ne_conected_to_me as $data_ctm) {
            foreach ($who_depends_i as $dependency_ne) {
                $dependency->addHostDependencyByDatas(
                    array(
                        'items_id' => $data_ctm['id'],
